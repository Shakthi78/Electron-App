...
function hideTaskbar() {
  exec(
    `"${nircmdPath}" win hide class Shell_TrayWnd`,
    (error, stdout, stderr) => {
      if (error) {
...
function showTaskbar() {
  exec(
    `"${nircmdPath}" win show class Shell_TrayWnd`,
    (error, stdout, stderr) => {
      if (error) {
...
const createMeetingWindow = (display, url, value) => {
  ...
  // Meeting controller starting function
    ...
    if (currentUrl.startsWith("https://exceleed.in/api/v1/auth/redirect") || currentUrl.startsWith("https://shakthi.online/api/v1/auth/microsoft/redirect")) {
      try {
        ...
        if (data.success && data.user) {
          ...
          if (secondaryWindow) {
            console.log(
              "🚀 Sending user email to secondary window:",
              data.user
            );
...
app.whenReady().then(async () => {
  ...
  secondaryDisplay = displays.find(
  ...
  if (secondaryDisplay) {
    secondaryWindow = createWindow(
      secondaryDisplay,
      path.join(app.getAppPath(), "/dist/secondary.html")
    );
  } else {
...
ipcMain.handle("get-volume", async () => {
  return new Promise((resolve, reject) => {
    ...
    exec(
      `powershell -command "Write-Output (New-Object -ComObject WMPlayer.OCX.7).settings.volume"`,
      (error, stdout) => {
...
ipcMain.on("close-meeting", async () => {
  if (secondaryWindow) {
    loadRouteInWindow(secondaryWindow, "/");
    try {
      await closeMeeting(); // 🆕 Close the Puppeteer Chrome instance
...
//   if (meetingWindow) {
//     meetingWindow.webContents
//       .executeJavaScript(`executeControl('${action}')`)
//       .catch((err) => console.error("Control failed:", err));
//   }
// });

ipcMain.on("meeting-control", async (event, action) => {
  console.log("Performing meeting action:", action);
